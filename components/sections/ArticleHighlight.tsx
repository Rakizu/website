...
export const ArticleHighlight: React.FC<ArticleHighlightProps> = ({ artikel }) => {
  ...
  useGSAP(() => {
    ...
    // Stagger reveal for the whole article item
    ...
    // Parallax inside the thumbnail when scrolled
    items.forEach(item => {
        ...
        <div className="flex flex-col divide-y divide-pure-surface/10">
          {artikel.map((a, i) => (
            <a 
              ...
              onClick={(e) => {
                e.preventDefault();
                window.dispatchEvent(new CustomEvent('navigate-curtain', { detail: { href: `/artikel/${a.id}`, isRoute: true } }));
              }}
            >
              <article
